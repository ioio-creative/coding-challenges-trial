...
class Bird {
  constructor(p5) {
    ...
    this.velocity = p5.createVector(
      p5.random(-this.maxSpeed, this.maxSpeed),
      p5.random(-this.maxSpeed, this.maxSpeed)
    );
    this.radius = 20;
  }
  ...
  outOfScreen() {
    if (this.position.x > w + this.radius) this.position.x = -this.radius;
    else if (this.position.x < -this.radius) this.position.x = w + this.radius;

    if (this.position.y > h + this.radius) this.position.y = -this.radius;
    else if (this.position.y < -this.radius) this.position.y = h + this.radius;
  }
